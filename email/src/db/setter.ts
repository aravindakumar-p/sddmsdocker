...
export const notificationsServices = async (
	services: any,
	schema: any,
	userid: any,
	subject: any,
	...
	try {
		...
		if (userid?.length > 0) {
			return userid.map(async (data: any) => {
				const notifiy = new NotificationsService({
					accountability: {
						admin: true,
					},
					schema: schema,
				});
				return await notifiy.createOne({
					recipient: data?.directus_users_id ? data?.directus_users_id : data,
					sender: sender,
					subject: subject,
					message,
