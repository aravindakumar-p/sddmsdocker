 ...
 * */
export default class QucikCilverController {
	get = null;
	set = null;
	services = null;
	...
	async placeOrder(reference_id:any, quantity:any, brand_sku:any, options:any, vendorOrderId:any, isOrderComplete:any,vendor_code:any) {
		try {
			...
			const checkBrandVendorMapping = await get.checkBrandVendorMapping(
				...
				amount,
				currency,
				config.vendor_id_mapping.QUICKCILVER_EGV,reference_id,vendor_code
			);
			...
			if (
				...
				const { vouchers, success, message, exception, orderId, status, statusCode, walletBalance } =
					await vendorVoucher.getQuickCilverVoucher(
						vendorSKU,
						quantity,
						reference_id,
	...
	async getOrderDetails(reference_id:any,vendorOrderId:any,vendor_code:any) {
		try {
			...
			const { vouchers, success, message, exception, orderId, status } = await vendorVoucher.getQCOlderOrder(
				reference_id,
	...
	async checkOrderStatus(reference_id:any,vendor_code:any) {
		try {
			...
			const response = await vendorVoucher.getQCOrderStatus(
				reference_id,
				vendor_code
			);
