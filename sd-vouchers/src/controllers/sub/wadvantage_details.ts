 ...
 * */
export default class WorkAdvantageDetails {
	get = null;
	set = null;
	services = null;
	...
	async getOrderDetails(reference_id: any,vendor_code:any) {
		try {
			...
			const { vouchers, success, message, exception, orderId } = await vendorVoucher.getWorkAdvantagePreviousOrder(
				reference_id,
				vendor_code
			);
	...
	async placeNewWaOrder(reference_id:any, quantity:any, brand_sku:any, vendor_code:any, options:any) {
		try {
			const get = new Getters(this.itemsService, this.accountabilitySchema);
			/* Vendor Vouchers Class to handle all aspects of retrieving vouchers from 3rd party apis */
			const vendorVoucher = new VendorVouchers();
			...
			if (vendorSkuMappingResponse && vendorSkuMappingResponse['vendor_sku']) {
				...
				const { vouchers, success, message, exception, orderId,vendorCode } = await vendorVoucher.getWorkAdvantageVoucher(
					vendorSKU,
					quantity,
					reference_id,
					retrieveIfRedeemed,
