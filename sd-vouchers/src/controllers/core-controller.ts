...
export default class CoreController {
	...
	async orderRewardStore({ vendor_code, reference_id, options, brand_sku, quantity, env }) {
		try {
			...
			const { vouchers, success, message, id, status ,exception , statusCode  }: any = await rewardController.placeOrder(
				appendedReferenceId,
				quantity,
				brand_sku,
				options,
			...
			const addVOrder = await set.updateVendorOrderStatus(
				reference_id,
				vendorOrderId ? vendorOrderId : id,
				success || isOrderComplete ? 'completed' : 'pending',
				vendor_code
			);

			const result = success
				? await rewardController.storeToInventory({ vouchers, reference_id, id, options, brand_sku, vendor_code })
				: null;
			const keys = result?.keys ?? null;
			const VoucherList = result?.VoucherList ?? null;
