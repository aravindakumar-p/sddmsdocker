...
export default class CoreController {
  ...
  getQCCatalog = async ({ countryCode }) => {
    try {
      ...
      await new LogSys().log(`getCatalog QC-AMAZON productList ${productList}`, false,null,null);
  ...
  getQuickCilverCatalog = async ({ countryCode, vendorApiIntId }) => {
    try {
      ...
      const category = vendorDetails ? vendorDetails['categoryid'] : null;
      /* Call get Catalog method of vendorVouchers */
  ...
  async oldVouchersWorkAdvantage({ vendor_code, reference_id, options }) {
    try {
      ...
      const result = success
        ? await workAdvantageDetails.storeToInventory({ vouchers, reference_id, orderId, options, brand_sku, vendor_code })
        : null;
  ...
  async oldVouchersQCAmazon({ vendor_code, reference_id, options }) {
    try {
      ...
      const addVOrder = await set.updateVendorOrderStatus(
        ...
        success ? 'completed' : 'pending', vendor_code
      );

     

      const result = success? await qcAmazonController.storeToInventory({ vouchers, reference_id, orderId, options, sku, vendor_code }) : null;
  ...
  async oldVouchersFlipkart({ vendor_code, reference_id, options }) {
    ...
    const egv: any = {};
      ...
      if (giftCardList === null || giftCardList.length ==0) {
        ...
        /* Update Order Status for Vendor */
        await set.updateVendorOrderStatus(
          reference_id,
          transactionId,
          "completed", 
          vendor_code
      ...
      const result = await flipkartController.storeToInventory({ giftCardList, reference_id, options,vendor_code, sku });
  ...
  async oldVouchersQuickCilver({ vendor_code, reference_id, options }) {
    try {
      ...
      const result = success? await qcController.storeToInventory({ vouchers, reference_id, orderId, options, sku, vendor_code }) : null;
  ...
  async orderWorkAdvantage({ vendor_code, reference_id, options, brand_sku, quantity }) {
    try {
      ...
      const result = success ? await workAdvantageDetails.storeToInventory({ vouchers, reference_id, orderId, options, brand_sku, vendor_code }): null;
  ...
  async orderQCAmazon({ vendor_code, reference_id, options, brand_sku, quantity, env }) {
    try {
      ...
      const addVOrder = await set.updateVendorOrderStatus(
        reference_id,
        vendorOrderId ? vendorOrderId : orderId,
        success || isOrderComplete ? 'completed' : 'pending', vendor_code
      );
      ...
      const result = success
        ? await qcAmazonController.storeToInventory({ vouchers, reference_id, orderId, options, brand_sku, vendor_code })
        : null;
  ...
  async orderFlipkart({ vendor_code, reference_id, options, brand_sku, quantity,env }) {
    try {
      ...
      let { giftCardList, success , message , exception, orderId, vendorCode} = await flipkartController.flipkartplaceOrder(
        reference_id,
        quantity,
        brand_sku,
        options,
  ...
  async getLinkVouchers({ link_reference_id, reference_code_otp, env }) {
    try {
      ...
      if (getResponse.length !== 0) {
        ...
        return await this.getLinkVoucherDetails({ sku, amount, link_reference_id, link_ledger_reference_id, env, soft_link_redeemed_status, inventoryId });
      } else {
        return {
          "success": false,
...
updateQuickCilverCatalogId = async ({ countryCode, vendorApiIntId }) => {
  try {
    ...
    await new LogSys().log(`id ${id}`, false,null,null);
     ...
     if (categoryId && id) {
        /* Update Order Status for Vendor */
         const getUpdateResponce = await set.updateVendorCategoryId(
           id,
           categoryId
         );
