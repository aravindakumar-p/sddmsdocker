...
export default class VendorVouchers {
	...
	getWorkAdvantageVoucher = async (upc_id:any, qty:any, referenceId:any, retrieveIfRedeemed:any,vendor_code:any) => {
		try {
			...
			if (isExceptionOccurred) {
				await new LogSys().log(
					...
					false,referenceId,vendor_code
				);
				return { vouchers: null, success: false, message: 'Exception Occurred:', orderId: null, exception: true,vendorCode:vendor_code};
			}else{
				return {
					vouchers: vouchers,
					success: success,
			...
			referenceId,
				vendor_code);
			return { vouchers: null, success: false, message: 'Exception Occurred:' + e, orderId: null,exception: true ,vendorCode:vendor_code};
		}
	};
	...
	async getWorkAdvantagePreviousOrder(referenceId:any,vendor_code:any) {
		try {
			...
			if (isExceptionOccurred) {
				...
				return { vouchers: null, success: false, message: 'Exception Occurred:', exception: true, orderId: orderId ,vendorCode:vendor_code};
	...
	getFlipkartVoucher = async (qty:any,reference_id:any,price:any,vendor_code:any,brand_sku:any) => {
		try {
			...
			if (success) {
				if (balance) {
					...
					for (let i = 0; i < getQty; i++) {
								...
								vendorOrderId = vendorOrderDetail && vendorOrderDetail['vendor_order_id'] ? vendorOrderDetail['vendor_order_id'] : null;
								...
								if (!vendorOrderDetail) {
									...
									if (transactionId) {
										...
										giftCardList.push(egvResponse);
											...
											if (egvResponse.codeException) {
												await new LogSys().log( `Gift Card  Generating  Error ${getTrasactionResponse}`, getTrasactionResponse,reference_id,vendor_code);
												commonMessge = egvResponse.statusCode
											    return {giftCardList:giftCardList,success:false, message: egvResponse.statusCode, codeException: true,vendorCode:vendor_code};
										...
										return {giftCardList:giftCardList,success:false, message: 'Flipkart TransactionId  Not Generated', codeException: true,vendorCode:vendor_code};
										...
										return {giftCardList:giftCardList,success:false, message: 'Transaction ID Not generated', codeException: true,vendorCode:vendor_code };
									}	
								} else {
									...
									egvDetails.statusMessage ='Please provide a unique reference ID for the order, following the gift card fetching process from the SD Gift Card Inventory.';
													...
													if (vendorOrderDetail.vendor_order_id) {
														...
														giftCardList.push(getEgvResponse);
															...
															if (getEgvResponse.codeException) {
																...
																return {giftCardList:giftCardList,success:false, message: getRedispatchFlipkartVoucher.statusMessage, codeException: true,vendorCode:vendor_code};
														...
														return {giftCardList:giftCardList,success:false, message: 'TransactionId Not Available', codeException: true,vendorCode:vendor_code};
													}
					...
					return {giftCardList:giftCardList,success:status, message: commonMessge, codeException: false,vendorCode:vendor_code};

				
					}else{
						await new LogSys().log(`Flikart Insufficient balance :${message}`, false,reference_id,vendor_code);
				        return {giftCardList:giftCardList,success:false, message: 'Flikart Insufficient balance', codeException: true,vendorCode:vendor_code };
					}
	          }else{
				await new LogSys().log(`Flikart Balance Not Found :${message}`, false,reference_id,vendor_code);

				return {giftCardList:giftCardList,success:false, message: 'Flikart Balance Not Found', codeException: true,vendorCode:vendor_code };
			...
			if (!codeException) {
				return {giftCardList:giftCardList,success:false, message: 'Flikart Balance Error', codeException: true,vendorCode:vendor_code };
	...
	getQCAmazonVoucher = async (
		...
		try {
			...
			referenceId,
				vendor_code);
			return { vouchers: null, success: false, message: 'Exception Occurred:' + e, exception: true, orderId: null,vendoCode:vendor_code };
		}
	};
	...
	getQCAmazonOlderOrder = async (referenceId:any, vendorOrderId:any,vendor_code:any) => {
		try {
			...
			return { vouchers: null, success: false, message: 'Exception Occurred:' + e, exception: true, orderId: null,vendorCode:vendor_code };
		}
	};
	...
	getQCAmazonOrderStatus = async (referenceId:any,vendor_code:any) => {
		try {
			...
			referenceId,
				vendor_code);
			return { vouchers: null, success: false, message: `Exception Occurred:${e}` , exception: true ,vendorCode:vendor_code};
		}
	};
